...
<body>
	...
	<div class="list_button_write">
		<select name="category" id="category" class="select_category" onchange="location.href=this.value">
			...
			<option name="category" value="/fortuneteller/index.jsp?page=reviewOotd.jsp" selected>오늘의 행운룩</option>
	...
	%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= startPage - pageBlock %>">[prev]&nbsp;</a>
		<%  }
			
			for(int i = startPage; i <= endPage; i++) {
				if(currentPage == i) {
		%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= i %>" class="currentpage"><%= i %></a>&nbsp;
			<%  } else { %>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= i %>"><%= i %></a>&nbsp;
			<%  } %>
		...
		<%	}
			...
			if(endPage < pageCount) {
		%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= startPage + pageBlock %>">&nbsp;[next]</a>
		<%		
			}
			...
			if(startPage > pageBlock) {
	%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%=startPage - pageBlock %>&searchCondition=<%= searchCondition %>&searchKeyword=<%=searchKeyword%>">[prev]&nbsp;</a>
		<%  }
			
			for(int i = startPage; i <= endPage; i++) {
				if(currentPage == i) {
		%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= i %>&searchCondition=<%= searchCondition %>&searchKeyword=<%=searchKeyword%>" class="currentpage"><%= i %></a>&nbsp;
			<%  } else { %>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=<%= i %>&searchCondition=<%= searchCondition %>&searchKeyword=<%=searchKeyword%>"><%= i %></a>&nbsp;
			<%  } %>
		...
		<%	}
			...
			if(endPage < pageCount) {
		%>
				<a href="/fortuneteller/index.jsp?page=reviewOotd.jsp?pageNum=%=startPage + pageBlock %>&searchCondition=<%= searchCondition %>&searchKeyword=<%=searchKeyword%>">&nbsp;[next]</a>
		<%		
			}
		}
		%>
